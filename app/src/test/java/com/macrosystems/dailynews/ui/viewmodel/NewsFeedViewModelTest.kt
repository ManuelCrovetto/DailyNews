...
class NewsFeedViewModelTest {
    ...
    @Test
    fun `viewStateFlow (MutableStateFlow), mutates state as desired when getNewsFeed returns a NOT EMPTY LIST`() = runTest {
        var counter = 1
        withContext(Dispatchers.Default) {
            ...
            newsFeedViewModel.viewState.test {
                while (counter < 3) {
                    ...
                    val state = awaitItem()
                    when(counter) {
                        1 -> { assert(state.isLoading && !state.success)}
                        2 -> { assert(!state.isLoading && state.success) }
                    }
                    counter += 1
                }
    ...
    @Test
    fun `viewStateFlow (MutableStateFlow), mutates state as desired when getNewsFeed returns a EMPTY LIST`() = runTest {
        var counter = 1
        withContext(Dispatchers.Default) {
            coEvery { getNewsFeed() } returns Result.Success(emptyList())
            newsFeedViewModel.viewState.test {
                while (counter < 3) {
                    ...
                    when (counter) {
                        1 -> { assert(state.isLoading && !state.success) }
                        2 -> { assert(!state.isLoading && state.success && state.isEmptyList) }
                    }
                    counter += 1
                }
                cancelAndConsumeRemainingEvents()
    ...
    @Test
    fun `viewStateFlow (MutableStateFlow), mutates state as desired when getNewsFeed returns ResultError with empty exception message`() = runTest {
        var counter = 1
        withContext(Dispatchers.Default) {
            coEvery { getNewsFeed() } returns Result.Error(Exception(""))
            newsFeedViewModel.viewState.test {
                while (counter < 3) {
                    newsFeedViewModel.getNewsFeedList()
                    val state = awaitItem()
    ...
    @Test
    fun `viewStateFlow (MutableStateFlow), mutates state as desired when getNewsFeed returns ResultError with exception message`() = runTest {
        var counter = 1
        withContext(Dispatchers.Default) {
            coEvery { getNewsFeed() } returns Result.Error(Exception("network error"))
            newsFeedViewModel.viewState.test {
                while (counter < 3) {
                    ...
                    when (counter) {
                        1 -> { assert(state.isLoading && !state.success) }
                        2 -> { assert(!state.isLoading && !state.success && state.error && state.isEmptyList) }
                    }
                    counter += 1
                }
                cancelAndConsumeRemainingEvents()
